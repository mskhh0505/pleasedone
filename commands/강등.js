...
exports.run = async (client, message, args) => {
    ...
    if(Number(maximumRank) <= rankInGroup){
        return message.channel.send({embed: {
            color: 16733013,
    ...
    let demoteResponse;
    try {
        demoteResponse = await roblox.demote(Number(groupId), id);
    } catch (err) {
    ...
    message.channel.send({embed: {
        ...
        fields: [
            {
                name: "전 계급",
                value: `${demoteResponse.oldRole.name}`,
                inline: false
            },
            {
                name: "새로운 계급",
                value: `${demoteResponse.newRole.name}`,
                inline: false
            },
            {
                name: "유저 ID",
